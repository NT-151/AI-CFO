    ...
    <Card className="bg-white rounded-xl shadow-sm border border-gray-200">
      ...
      <CardContent>
        <div className="space-y-8">
          {" "}
          {/* Increased from space-y-6 to space-y-8 */}
          {integrations.slice(0, 2).map((integration: Integration) => {
            const config = getIntegrationConfig(integration.platform);
            const Icon = config.icon;
            return (
              <div
                key={integration.platform}
                className="bg-white rounded-lg p-8 shadow-sm border border-gray-200" // Increased padding from p-6 to p-8
              >
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-semibold">
                      {formatLastSync(integration.lastSync || null)}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="font-semibold">
                      {integration.platform === "payabl" &&
                        `${integration.metadata?.accounts || "0"} accounts`}
                      {integration.platform === "ipushpull" &&
                        `${integration.metadata?.streams || "0"} active feeds`}
                    </div>
                  </div>
                </div>
              </div>
        ...
        <Button className="w-full mt-4 bg-google-blue hover:bg-google-blue-dark">
          Manage Integrations
        </Button>
      </CardContent>
    </Card>
