...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const success = await register(username, password, companyName);

      if (success) {
        // Only clear fields and redirect on successful registration
        setUsername("");
        setPassword("");
        setCompanyName("");
        setLocation("/");
        ...
        // Don't clear fields on failed registration for better UX
      }
    } catch (error) {
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            Join FiscalGenius and start your financial journey
          </p>
        </div>
